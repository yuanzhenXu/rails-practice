...
class VariantsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create variant" do
    assert_difference('Variant.count') do
      post variants_url, params: { variant: { price: @variant.price, product_id: @variant.product_id, size: @variant.size } }
    end
  ...
  test "should update variant" do
    patch variant_url(@variant), params: { variant: { price: @variant.price, product_id: @variant.product_id, size: @variant.size } }
